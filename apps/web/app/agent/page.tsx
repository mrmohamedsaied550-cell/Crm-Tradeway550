...
import { Calendar, ClipboardList, MessagesSquare, ArrowUpRight } from 'lucide-react';

/**
 * Agent landing page (C31).
 *
 * P3-04 — adds the Calendar surface alongside My Leads + WhatsApp Inbox.
 */
export default function AgentPage() {
  const tAgent = useTranslations('agent');
  const tWorkspace = useTranslations('agent.workspace');
  const tInbox = useTranslations('agent.inbox');
  const tCalendar = useTranslations('agent.calendar');
        ...
        <li>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-ink-primary">{tCalendar('title')}</span>
            </div>
            <p className="text-xs text-ink-secondary">{tCalendar('subtitle')}</p>
            <span className="mt-auto inline-flex items-center gap-1 text-xs font-medium text-brand-700 group-hover:text-brand-800">
              {tCalendar('open')}
              <ArrowUpRight className="h-3.5 w-3.5" aria-hidden="true" />
            </span>
